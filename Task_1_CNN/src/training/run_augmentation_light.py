"""
Execution Entry Point for Experiment 2A: Light Data Augmentation (Random Horizontal Flip ONLY).

This script executes the complete Part B Experiment 2A training pipeline on CIFAR-10:
...
4. Constructs tf.data input pipelines with Light Data Augmentation (Horizontal Flip ONLY) for training
...
8. Persists checkpoint (augmentation_light_cifar10_best.keras) and logs
   (augmentation_light_training_log.csv, augmentation_light_training_history.json).
9. Generates convergence plots (figures/augmentation_light_training_curves.png).
10. Saves structured metrics summary (results/metrics/augmentation_light_experiment_summary.json)
    and prints summary table.

Usage:
    Direct execution:
        python Task_1_CNN/src/training/run_augmentation_light.py

    Module execution (from Task_1_CNN directory):
        python -m src.training.run_augmentation_light

    Module execution (from workspace root):
        python -m Task_1_CNN.src.training.run_augmentation_light
"""
...
from src.data.transforms import create_augmented_tf_datasets, get_light_augmentation_pipeline
...
logger = logging.getLogger("run_augmentation_light")


def verify_baseline_architecture(model: tf.keras.Model) -> None:
    """
    Perform structural and guardrail verification on the Baseline CNN model for Experiment 2A.
...
def run_augmentation_light_experiment() -> Tuple[tf.keras.Model, tf.keras.callbacks.History, Dict[str, Any]]:
    """
    Execute the end-to-end Experiment 2A (Light Data Augmentation — Horizontal Flip ONLY) on CIFAR-10.
    ...
    """
    print("\n" + "=" * 80)
    print("      TASK 1 PART B: EXPERIMENT 2A — LIGHT DATA AUGMENTATION (HORIZONTAL FLIP ONLY)")
    print("=" * 80)
    ...
    logger.info("Constructing tf.data input pipelines with Light Augmentation (Horizontal Flip ONLY)...")
    ...
    print(f"  • Augmentation     : Random Horizontal Flip ONLY (mode='horizontal', seed={RANDOM_SEED})")
    print(f"  • Aug Scope        : Applied to Training data ONLY (Val and Test remain unaugmented)")
    print(f"  • Other Transforms : Rotation=0, Shift=0, Zoom=0, Brightness=0, Contrast=0, Crop=None")

    # 6. Instantiate Trainer
    logger.info("Initializing Trainer for Experiment 2A...")
    ...
    # Confirm non-collision with existing baseline, dropout, batchnorm, and L2 files
    existing_ckpts = [
    ...
    print("\n" + "=" * 80)
    print("        STARTING 30-EPOCH LIGHT AUGMENTATION EXPERIMENT TRAINING")
    print("=" * 80)
    ...
    plot_training_history(
        ...
        title="CIFAR-10 CNN with Light Data Augmentation (Horizontal Flip) - Convergence",
    )

    # 12. Print Concise Experiment Summary Table
    print("\n" + "=" * 80)
    print("      EXPERIMENT 2A (LIGHT DATA AUGMENTATION) SUMMARY")
    print("=" * 80)
    print(f"  • Experiment Name          : {experiment_name}")
    print(f"  • Augmentation Level       : {augmentation_level.upper()} (Horizontal Flip ONLY)")
    print(f"  • Horizontal Flip          : Enabled (Random)")
    print(f"  • Other Transforms         : Disabled (Rotation=0, Shift=0, Zoom=0, Brightness=0, Contrast=0)")
...
if __name__ == "__main__":
    run_augmentation_light_experiment()
