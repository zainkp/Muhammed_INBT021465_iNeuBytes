"""
Standalone Smoke & Structural Verification for Task 1 Part B — Experiment 2C (Aggressive Data Augmentation).
...
8. Augmentation pipeline configuration: Exact aggressive controls
   - Random horizontal flip enabled
   - RandomRotation factor: exactly 0.15 (±54 degrees)
   - RandomTranslation height_factor: exactly 0.10 (±10% / ~3.2px)
   - RandomTranslation width_factor: exactly 0.10 (±10% / ~3.2px)
   - RandomZoom height/width factor: exactly 0.20 (±20%)
   - RandomBrightness factor: exactly 0.20 (±20%, value_range=(0.0, 1.0))
   - RandomContrast factor: exactly 0.20 (±20%)
   - Crop: false, Resize: false
...
14. Target artifact paths distinction (zero overwriting of baseline, dropout, batchnorm, L2, light, or moderate artifacts)
"""
...
from src.data.transforms import (
    ...
    get_moderate_augmentation_pipeline,
    verify_aggressive_augmentation_config,
    verify_light_augmentation_config,
    verify_moderate_augmentation_config,
)
...
logger = logging.getLogger("verify_augmentation_aggressive")


def run_smoke_verification() -> bool:
    print("=" * 80)
    print("      EXPERIMENT 2C: PRE-TRAINING SMOKE & STRUCTURAL VERIFICATION")
    print("=" * 80)

    set_seed(RANDOM_SEED)

    # --------------------------------------------------------------------------
    # 1. Configuration File & Cross-Experiment Preservation Verification
    # --------------------------------------------------------------------------
    print("\n[Step 1/6] Validating YAML Configuration Files & Cross-Experiment Preservation...")
    
    # 1A. Baseline Config Preservation Check
    baseline_cfg_path = CONFIGS_DIR / "baseline_config.yaml"
    assert baseline_cfg_path.exists(), f"Missing baseline configuration file: {baseline_cfg_path}"
    with open(baseline_cfg_path, "r", encoding="utf-8") as f:
        baseline_cfg = yaml.safe_load(f)
    assert baseline_cfg.get("seed") == 42, "Baseline seed mismatch"
    assert baseline_cfg.get("dataset", {}).get("train_samples") == 40000, "Baseline train samples mismatch"
    assert baseline_cfg.get("dataset", {}).get("val_samples") == 10000, "Baseline val samples mismatch"
    assert baseline_cfg.get("dataset", {}).get("test_samples") == 10000, "Baseline test samples mismatch"
    print("  • Baseline Config Check        : [PASS] (baseline_config.yaml preserved & verified)")

    # 1B. Experiment 2A Light Config Preservation Check
    light_cfg_path = CONFIGS_DIR / "augmentation_light_config.yaml"
    assert light_cfg_path.exists(), f"Missing 2A configuration file: {light_cfg_path}"
    with open(light_cfg_path, "r", encoding="utf-8") as f:
        light_cfg = yaml.safe_load(f)
    assert light_cfg.get("experiment_name") == "augmentation_light_cifar10", "2A experiment name mismatch"
    assert light_cfg.get("augmentation_level") == "light", "2A augmentation level mismatch"
    assert light_cfg.get("seed") == 42, "2A seed mismatch"
    verify_light_augmentation_config(light_cfg.get("augmentation", {}))
    print("  • 2A Light Config Check        : [PASS] (augmentation_light_config.yaml preserved & verified)")

    # 1C. Experiment 2B Moderate Config Preservation Check
    mod_cfg_path = CONFIGS_DIR / "augmentation_moderate_config.yaml"
    assert mod_cfg_path.exists(), f"Missing 2B configuration file: {mod_cfg_path}"
    with open(mod_cfg_path, "r", encoding="utf-8") as f:
        mod_cfg = yaml.safe_load(f)
    assert mod_cfg.get("experiment_name") == "augmentation_moderate_cifar10", "2B experiment name mismatch"
    assert mod_cfg.get("augmentation_level") == "moderate", "2B augmentation level mismatch"
    assert mod_cfg.get("seed") == 42, "2B seed mismatch"
    verify_moderate_augmentation_config(mod_cfg.get("augmentation", {}))
    print("  • 2B Moderate Config Check     : [PASS] (augmentation_moderate_config.yaml preserved & verified)")

    # 1D. Experiment 2C Aggressive Config Verification
    config_path = CONFIGS_DIR / "augmentation_aggressive_config.yaml"
    ...
    assert config.get("experiment_name") == "augmentation_aggressive_cifar10", "Experiment name mismatch in config"
    assert config.get("experiment_type") == "augmentation", "Experiment type mismatch in config"
    assert config.get("augmentation_level") == "aggressive", "Augmentation level mismatch in config"
    assert config.get("seed") == 42, "Seed mismatch in config"

    # Verify exact augmentation parameters in config
    aug_cfg = config.get("augmentation", {})
    verify_aggressive_augmentation_config(aug_cfg)

    # Strict equality assertions for Experiment 2C exact controls
    assert aug_cfg.get("horizontal_flip") is True, "horizontal_flip must be True"
    assert abs(float(aug_cfg.get("rotation_factor", 0.0)) - 0.15) < 1e-6, "rotation_factor must be exactly 0.15 (±54°)"
    assert abs(float(aug_cfg.get("height_shift_factor", 0.0)) - 0.10) < 1e-6, "height_shift_factor must be exactly 0.10 (±10%)"
    assert abs(float(aug_cfg.get("width_shift_factor", 0.0)) - 0.10) < 1e-6, "width_shift_factor must be exactly 0.10 (±10%)"
    assert abs(float(aug_cfg.get("zoom_factor", 0.0)) - 0.20) < 1e-6, "zoom_factor must be exactly 0.20 (±20%)"
    assert abs(float(aug_cfg.get("brightness_factor", 0.0)) - 0.20) < 1e-6, "brightness_factor must be exactly 0.20 (±20%)"
    assert abs(float(aug_cfg.get("contrast_factor", 0.0)) - 0.20) < 1e-6, "contrast_factor must be exactly 0.20 (±20%)"
    assert aug_cfg.get("crop") is False, "crop must be False"
    assert aug_cfg.get("resize") is False, "resize must be False"
    assert aug_cfg.get("apply_to") == "train_only", "apply_to must be 'train_only'"
    assert aug_cfg.get("validation_augmented") is False, "validation_augmented must be False"
    assert aug_cfg.get("test_augmented") is False, "test_augmented must be False"

    print("  • 2C Aggressive Config Check   : [PASS] (Flip=True, Rot=0.15 [±54°], Shift=0.10 [±10%], Zoom=0.20, Brightness=0.20, Contrast=0.20)")
    ...
    assert len(dropout_layers) == 0, f"Found {len(dropout_layers)} Dropout layers; must be 0 for Experiment 2C"
    assert len(bn_layers) == 0, f"Found {len(bn_layers)} BatchNorm layers; must be 0 for Experiment 2C"
    ...
    print("  • Regularizer Absence Check    : [PASS] (0 L1/L2 weight decay regularizers detected)")
    ...
    print("  • Parameter Count Check        : [PASS] (Exact match: 2,658,122 parameters)")

    # --------------------------------------------------------------------------
    # 4. Augmentation Pipeline & Dynamic Smoke Tests
    # --------------------------------------------------------------------------
    print("\n[Step 4/6] Executing Aggressive Augmentation Pipeline & Model Forward-Pass Smoke Tests...")
    aug_pipeline = get_aggressive_augmentation_pipeline(
        rotation_factor=0.15,
        height_shift_factor=0.10,
        width_shift_factor=0.10,
        zoom_factor=0.20,
    ...
    assert len(aug_pipeline.layers) == 6, f"Expected 6 layers in pipeline, found {len(aug_pipeline.layers)}"
    assert isinstance(aug_pipeline.layers[0], tf.keras.layers.RandomFlip), "Expected RandomFlip layer"
    assert isinstance(aug_pipeline.layers[1], tf.keras.layers.RandomRotation), "Expected RandomRotation layer"
    assert isinstance(aug_pipeline.layers[2], tf.keras.layers.RandomTranslation), "Expected RandomTranslation layer"
    assert isinstance(aug_pipeline.layers[3], tf.keras.layers.RandomZoom), "Expected RandomZoom layer"
    assert isinstance(aug_pipeline.layers[4], tf.keras.layers.RandomBrightness), "Expected RandomBrightness layer"
    assert isinstance(aug_pipeline.layers[5], tf.keras.layers.RandomContrast), "Expected RandomContrast layer"
    ...
    # In training=False mode, preprocessing layers should return identical tensors
    np.testing.assert_allclose(unaug_output.numpy(), dummy_input.numpy(), atol=1e-6, err_msg="Inference mode should not alter images")
    print("  • Augmentation Layer Behavior  : [PASS] (Active on training=True, Inactive/Identity on training=False)")
    ...
    print("  • CNN Model Forward Pass       : [PASS] (Output shape (4, 10) confirmed for training and inference)")
    ...
    print("  • Frozen Split Integrity Check : [PASS] (Exact 40k/10k/10k sample counts confirmed)")
    ...
    print("  • Validation Unaugmented Check : [PASS] (Validation images identical to raw slice)")
    ...
    trainer = BaselineTrainer(
        ...
        checkpoint_filename="augmentation_aggressive_cifar10_best.keras",
        logs_dir=LOGS_DIR,
        history_filename="augmentation_aggressive_training_history.json",
        csv_log_filename="augmentation_aggressive_training_log.csv",
        experiment_name="augmentation_aggressive_cifar10",
    )
    ...
    aug_light_ckpt = CHECKPOINTS_DIR / "augmentation_light_cifar10_best.keras"
    aug_light_csv = LOGS_DIR / "augmentation_light_training_log.csv"
    aug_light_json = LOGS_DIR / "augmentation_light_training_history.json"

    aug_mod_ckpt = CHECKPOINTS_DIR / "augmentation_moderate_cifar10_best.keras"
    aug_mod_csv = LOGS_DIR / "augmentation_moderate_training_log.csv"
    aug_mod_json = LOGS_DIR / "augmentation_moderate_training_history.json"
    ...
    assert aug_light_ckpt.exists(), f"Missing light augmentation checkpoint: {aug_light_ckpt}"
    assert aug_light_csv.exists(), f"Missing light augmentation CSV: {aug_light_csv}"
    assert aug_light_json.exists(), f"Missing light augmentation JSON: {aug_light_json}"

    assert aug_mod_ckpt.exists(), f"Missing moderate augmentation checkpoint: {aug_mod_ckpt}"
    assert aug_mod_csv.exists(), f"Missing moderate augmentation CSV: {aug_mod_csv}"
    assert aug_mod_json.exists(), f"Missing moderate augmentation JSON: {aug_mod_json}"

    # Confirm augmentation aggressive target paths are completely distinct
    existing_ckpts = [baseline_ckpt, dropout_ckpt, batchnorm_ckpt, l2_ckpt, aug_light_ckpt, aug_mod_ckpt]
    existing_csvs = [baseline_csv, dropout_csv, batchnorm_csv, l2_csv, aug_light_csv, aug_mod_csv]
    existing_jsons = [baseline_json, dropout_json, batchnorm_json, l2_json, aug_light_json, aug_mod_json]
    ...
    print(f"  • Preserved Aug Light Checkpoint  : {aug_light_ckpt.name}")
    print(f"  • Preserved Aug Mod Checkpoint    : {aug_mod_ckpt.name}")
    print(f"  • Target Aug Aggressive Checkpoint: {trainer.checkpoint_filepath.name}")
    print(f"  • Target Aug Aggressive CSV Log   : {trainer.csv_log_filepath.name}")
    print(f"  • Target Aug Aggressive JSON Log  : {trainer.history_filepath.name}")
    print("  • Artifact Path Collision Check   : [PASS] (Zero collision with existing artifacts)")
