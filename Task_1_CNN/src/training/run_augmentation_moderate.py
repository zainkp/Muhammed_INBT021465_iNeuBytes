"""
Execution Entry Point for Experiment 2B: Moderate Data Augmentation (Horizontal Flip + Rotation + Translation).

This script executes the complete Part B Experiment 2B training pipeline on CIFAR-10:
...
4. Constructs tf.data input pipelines with Moderate Data Augmentation:
   - Random Horizontal Flip
   - Small Random Rotation (factor=0.05, ±18 degrees)
   - Small Random Translation (height_factor=0.05, width_factor=0.05, ±5% / ~1.6px)
...
8. Persists checkpoint (augmentation_moderate_cifar10_best.keras) and logs
   (augmentation_moderate_training_log.csv, augmentation_moderate_training_history.json).
9. Generates convergence plots (figures/augmentation_moderate_training_curves.png).
10. Saves structured metrics summary (results/metrics/augmentation_moderate_experiment_summary.json)
    and prints summary table.

Usage:
    Direct execution:
        python Task_1_CNN/src/training/run_augmentation_moderate.py

    Module execution (from Task_1_CNN directory):
        python -m src.training.run_augmentation_moderate

    Module execution (from workspace root):
        python -m Task_1_CNN.src.training.run_augmentation_moderate
"""
...
from src.data.transforms import create_augmented_tf_datasets, get_moderate_augmentation_pipeline
...
logger = logging.getLogger("run_augmentation_moderate")


def verify_baseline_architecture(model: tf.keras.Model) -> None:
    """
    Perform structural and guardrail verification on the Baseline CNN model for Experiment 2B.
...
def run_augmentation_moderate_experiment() -> Tuple[tf.keras.Model, tf.keras.callbacks.History, Dict[str, Any]]:
    """
    Execute the end-to-end Experiment 2B (Moderate Data Augmentation — Flip + Rotation + Translation) on CIFAR-10.
    ...
    """
    print("\n" + "=" * 80)
    print("      TASK 1 PART B: EXPERIMENT 2B — MODERATE DATA AUGMENTATION")
    print("      (RANDOM HORIZONTAL FLIP + ROTATION ±18° + TRANSLATION ±5% / ~1.6px)")
    print("=" * 80)
    ...
    summary_filename = "augmentation_moderate_experiment_summary.json"
    curves_filename = "augmentation_moderate_training_curves.png"
    ...
    logger.info("Constructing tf.data input pipelines with Moderate Augmentation (Flip + Rotation 0.05 + Translation 0.05)...")
    ...
    print(f"  • Rotation         : Random Rotation (factor=0.05, ±18°)")
    print(f"  • Translation      : Random Translation (height=0.05, width=0.05, ±5% / ~1.6px)")
    print(f"  • Aug Scope        : Applied to Training data ONLY (Val and Test remain unaugmented)")
    print(f"  • Other Transforms : Zoom=0, Brightness=0, Contrast=0, Crop=None, Resize=None")

    # 6. Instantiate Trainer
    logger.info("Initializing Trainer for Experiment 2B...")
    ...
    # Confirm non-collision with existing baseline, dropout, batchnorm, l2, and light files
    existing_ckpts = [
    ...
    print("\n" + "=" * 80)
    print("        STARTING 30-EPOCH MODERATE AUGMENTATION EXPERIMENT TRAINING")
    print("=" * 80)
    ...
    plot_training_history(
        ...
        title="CIFAR-10 CNN with Moderate Data Augmentation (Flip + Rot 0.05 + Trans 0.05) - Convergence",
    )

    # 12. Print Concise Experiment Summary Table
    print("\n" + "=" * 80)
    print("      EXPERIMENT 2B (MODERATE DATA AUGMENTATION) SUMMARY")
    print("=" * 80)
    print(f"  • Experiment Name          : {experiment_name}")
    print(f"  • Augmentation Level       : {augmentation_level.upper()} (Flip + Rotation ±18° + Translation ±5%)")
    print(f"  • Horizontal Flip          : Enabled (Random)")
    print(f"  • Rotation                 : factor=0.05 (±18°)")
    print(f"  • Translation              : height=0.05, width=0.05 (±5% / ~1.6px)")
    print(f"  • Other Transforms         : Disabled (Zoom=0, Brightness=0, Contrast=0, Crop=None)")
...
if __name__ == "__main__":
    run_augmentation_moderate_experiment()
