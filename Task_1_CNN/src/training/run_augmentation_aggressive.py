"""
Execution Entry Point for Experiment 2C: Aggressive Data Augmentation (Flip + Rotation + Translation + Zoom + Brightness + Contrast).

This script executes the complete Part B Experiment 2C training pipeline on CIFAR-10:
...
4. Constructs tf.data input pipelines with Aggressive Data Augmentation:
   - Random Horizontal Flip
   - Heavy Random Rotation (factor=0.15, ±54 degrees)
   - Random Translation (height_factor=0.10, width_factor=0.10, ±10% / ~3.2px)
   - Random Zoom (height_factor=0.20, width_factor=0.20, ±20%)
   - Random Brightness (factor=0.20, value_range=(0.0, 1.0), ±20%)
   - Random Contrast (factor=0.20, ±20%)
   and unaugmented input for validation.
5. Instantiates the uncompiled Baseline CNN architecture (2,658,122 parameters, no Dropout/BatchNorm/L1/L2).
6. Validates baseline architecture integrity and parameter count.
7. Compiles and executes the fixed 30-epoch training protocol.
8. Persists checkpoint (augmentation_aggressive_cifar10_best.keras) and logs
   (augmentation_aggressive_training_log.csv, augmentation_aggressive_training_history.json).
9. Generates convergence plots (figures/augmentation_aggressive_training_curves.png).
10. Saves structured metrics summary (results/metrics/augmentation_aggressive_experiment_summary.json)
    and prints summary table.

Usage:
    Direct execution:
        python Task_1_CNN/src/training/run_augmentation_aggressive.py

    Module execution (from Task_1_CNN directory):
        python -m src.training.run_augmentation_aggressive

    Module execution (from workspace root):
        python -m Task_1_CNN.src.training.run_augmentation_aggressive
"""
...
logger = logging.getLogger("run_augmentation_aggressive")


def verify_baseline_architecture(model: tf.keras.Model) -> None:
    """
    Perform structural and guardrail verification on the Baseline CNN model for Experiment 2C.
    ...
    """
    logger.info("Performing structural verification on Baseline CNN model...")
    ...
    # 3. Verify absence of weight regularizers
    for layer in model.layers:
        assert getattr(layer, "kernel_regularizer", None) is None, (
            f"Layer '{layer.name}' has kernel_regularizer, violating experiment constraints."
        )
    ...
    logger.info("[SUCCESS] Architecture verified: Exact baseline CNN (2,658,122 parameters, 0 regularization).")


def run_augmentation_aggressive_experiment() -> Tuple[tf.keras.Model, tf.keras.callbacks.History, Dict[str, Any]]:
    """
    Execute the end-to-end Experiment 2C (Aggressive Data Augmentation) on CIFAR-10.
    ...
    """
    print("\n" + "=" * 80)
    print("      TASK 1 PART B: EXPERIMENT 2C — AGGRESSIVE DATA AUGMENTATION")
    print("      (FLIP + ROTATION ±54° + TRANSLATION ±10% + ZOOM ±20% + BRIGHTNESS ±20% + CONTRAST ±20%)")
    print("=" * 80)
    ...
    summary_filename = "augmentation_aggressive_experiment_summary.json"
    curves_filename = "augmentation_aggressive_training_curves.png"
    ...
    # 3. Build tf.data input pipelines (augmented train, unaugmented val, isolated test)
    logger.info("Constructing tf.data input pipelines with Aggressive Augmentation (Flip + Rot 0.15 + Trans 0.10 + Zoom 0.20 + Bright 0.20 + Contrast 0.20)...")
    ...
    train_ds, val_ds, _ = create_augmented_tf_datasets(
        x_train=x_train,
        y_train=y_train,
        x_val=x_val,
        y_val=y_val,
    ...
    verify_baseline_architecture(model)

    print(f"\n[Model Architecture & Augmentation Specification]")
    ...
    print(f"  • Augmentation     : Random Horizontal Flip (mode='horizontal')")
    print(f"  • Rotation         : Random Rotation (factor=0.15, ±54°)")
    print(f"  • Translation      : Random Translation (height=0.10, width=0.10, ±10% / ~3.2px)")
    print(f"  • Zoom             : Random Zoom (height=0.20, width=0.20, ±20%)")
    print(f"  • Brightness       : Random Brightness (factor=0.20, value_range=(0.0, 1.0), ±20%)")
    print(f"  • Contrast         : Random Contrast (factor=0.20, ±20%)")
    print(f"  • Aug Scope        : Applied to Training data ONLY (Val and Test remain unaugmented)")
    print(f"  • Other Transforms : Crop=None, Resize=None")

    # 6. Instantiate Trainer
    logger.info("Initializing Trainer for Experiment 2C...")
    ...
    # Confirm non-collision with existing baseline, dropout, batchnorm, l2, light, and moderate files
    existing_ckpts = [
        CHECKPOINTS_DIR / "baseline_cifar10_best.keras",
        CHECKPOINTS_DIR / "dropout_cifar10_best.keras",
        CHECKPOINTS_DIR / "batchnorm_cifar10_best.keras",
        CHECKPOINTS_DIR / "l2_cifar10_best.keras",
        CHECKPOINTS_DIR / "augmentation_light_cifar10_best.keras",
        CHECKPOINTS_DIR / "augmentation_moderate_cifar10_best.keras",
    ]
    existing_csvs = [
        LOGS_DIR / "baseline_training_log.csv",
        LOGS_DIR / "dropout_training_log.csv",
        LOGS_DIR / "batchnorm_training_log.csv",
        LOGS_DIR / "l2_training_log.csv",
        LOGS_DIR / "augmentation_light_training_log.csv",
        LOGS_DIR / "augmentation_moderate_training_log.csv",
    ]
    existing_jsons = [
        LOGS_DIR / "baseline_training_history.json",
        LOGS_DIR / "dropout_training_history.json",
        LOGS_DIR / "batchnorm_training_history.json",
        LOGS_DIR / "l2_training_history.json",
        LOGS_DIR / "augmentation_light_training_history.json",
        LOGS_DIR / "augmentation_moderate_training_history.json",
    ]
    ...
    print("\n" + "=" * 80)
    print("        STARTING 30-EPOCH AGGRESSIVE AUGMENTATION EXPERIMENT TRAINING")
    print("=" * 80)
    ...
    plot_training_history(
        ...
        title="CIFAR-10 CNN with Aggressive Data Augmentation (Flip + Rot 0.15 + Trans 0.10 + Zoom 0.20 + Bright 0.20 + Contrast 0.20) - Convergence",
    )

    # 12. Print Concise Experiment Summary Table
    print("\n" + "=" * 80)
    print("      EXPERIMENT 2C (AGGRESSIVE DATA AUGMENTATION) SUMMARY")
    print("=" * 80)
    print(f"  • Experiment Name          : {experiment_name}")
    print(f"  • Augmentation Level       : {augmentation_level.upper()} (Flip + Rot ±54° + Trans ±10% + Zoom ±20% + Bright ±20% + Contrast ±20%)")
    print(f"  • Horizontal Flip          : Enabled (Random)")
    print(f"  • Rotation                 : factor=0.15 (±54°)")
    print(f"  • Translation              : height=0.10, width=0.10 (±10% / ~3.2px)")
    print(f"  • Zoom                     : height=0.20, width=0.20 (±20%)")
    print(f"  • Brightness               : factor=0.20 (±20%, value_range=(0.0, 1.0))")
    print(f"  • Contrast                 : factor=0.20 (±20%)")
...
if __name__ == "__main__":
    run_augmentation_aggressive_experiment()
