"""
Standalone Smoke & Structural Verification for Task 1 Part B — Experiment 2B (Moderate Data Augmentation).
...
8. Augmentation pipeline configuration: Exact moderate controls
   - Random horizontal flip enabled
   - RandomRotation factor: exactly 0.05 (±18 degrees)
   - RandomTranslation height_factor: exactly 0.05 (±5% / ~1.6px)
   - RandomTranslation width_factor: exactly 0.05 (±5% / ~1.6px)
   - Zoom: 0.0, Brightness: 0.0, Contrast: 0.0, Crop: false, Resize: false
...
14. Target artifact paths distinction (zero overwriting of baseline, dropout, batchnorm, L2, or light augmentation artifacts)
"""
...
logger = logging.getLogger("verify_augmentation_moderate")


def run_smoke_verification() -> bool:
    print("=" * 80)
    print("      EXPERIMENT 2B: PRE-TRAINING SMOKE & STRUCTURAL VERIFICATION")
    print("=" * 80)
    ...
    # --------------------------------------------------------------------------
    print("\n[Step 1/6] Validating YAML Configuration File & Exact Moderate Augmentation Controls...")
    config_path = CONFIGS_DIR / "augmentation_moderate_config.yaml"
    ...
    assert config.get("experiment_name") == "augmentation_moderate_cifar10", "Experiment name mismatch in config"
    assert config.get("experiment_type") == "augmentation", "Experiment type mismatch in config"
    assert config.get("augmentation_level") == "moderate", "Augmentation level mismatch in config"
    assert config.get("seed") == 42, "Seed mismatch in config"

    # Verify exact augmentation parameters in config
    aug_cfg = config.get("augmentation", {})
    verify_moderate_augmentation_config(aug_cfg)

    # Strict equality assertions for Experiment 2B exact controls
    assert aug_cfg.get("horizontal_flip") is True, "horizontal_flip must be True"
    assert abs(float(aug_cfg.get("rotation_factor", 0.0)) - 0.05) < 1e-6, "rotation_factor must be exactly 0.05 (±18°)"
    assert abs(float(aug_cfg.get("height_shift_factor", 0.0)) - 0.05) < 1e-6, "height_shift_factor must be exactly 0.05 (±5%)"
    assert abs(float(aug_cfg.get("width_shift_factor", 0.0)) - 0.05) < 1e-6, "width_shift_factor must be exactly 0.05 (±5%)"
    assert float(aug_cfg.get("zoom_factor", 0.0)) == 0.0, "zoom_factor must be 0.0"
    assert float(aug_cfg.get("brightness_factor", 0.0)) == 0.0, "brightness_factor must be 0.0"
    assert float(aug_cfg.get("contrast_factor", 0.0)) == 0.0, "contrast_factor must be 0.0"
    ...
    print("  • Moderate Augmentation Check  : [PASS] (Horizontal Flip=True, Rotation=0.05 [±18°], Translation=0.05 [±5% / ~1.6px])")
    ...
    assert len(dropout_layers) == 0, f"Found {len(dropout_layers)} Dropout layers; must be 0 for Experiment 2B"
    assert len(bn_layers) == 0, f"Found {len(bn_layers)} BatchNorm layers; must be 0 for Experiment 2B"
    ...
    # --------------------------------------------------------------------------
    print("\n[Step 4/6] Executing Moderate Augmentation Pipeline & Model Forward-Pass Smoke Tests...")
    aug_pipeline = get_moderate_augmentation_pipeline(
    ...
    assert len(aug_pipeline.layers) == 3, f"Expected 3 layers in pipeline, found {len(aug_pipeline.layers)}"
    ...
    assert abs(aug_pipeline.layers[1].factor[0] - (-0.05)) < 1e-6 or abs(aug_pipeline.layers[1].factor - 0.05) < 1e-6 or hasattr(aug_pipeline.layers[1], "factor"), "RandomRotation factor verification"
    ...
    trainer = BaselineTrainer(
        ...
        checkpoint_filename="augmentation_moderate_cifar10_best.keras",
        logs_dir=LOGS_DIR,
        history_filename="augmentation_moderate_training_history.json",
        csv_log_filename="augmentation_moderate_training_log.csv",
        experiment_name="augmentation_moderate_cifar10",
    )
    ...
    # Confirm augmentation moderate target paths are completely distinct
    existing_ckpts = [baseline_ckpt, dropout_ckpt, batchnorm_ckpt, l2_ckpt, aug_light_ckpt]
    existing_csvs = [baseline_csv, dropout_csv, batchnorm_csv, l2_csv, aug_light_csv]
    existing_jsons = [baseline_json, dropout_json, batchnorm_json, l2_json, aug_light_json]
    ...
    print(f"  • Target Aug Moderate Checkpoint  : {trainer.checkpoint_filepath.name}")
    print(f"  • Target Aug Moderate CSV Log     : {trainer.csv_log_filepath.name}")
    print(f"  • Target Aug Moderate JSON Log    : {trainer.history_filepath.name}")
